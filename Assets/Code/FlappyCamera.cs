...
public class FlappyCamera : MonoBehaviour 
{
	...
	const int DESTROY_OBSTACLE_OFFSET = 10;
    ...
    void FixedUpdate() 
	{
        ...
        UpdatePosition();
        UpdateFOV();
		if (FlappyObstaclesSpawner.getObstacleX() + DESTROY_OBSTACLE_OFFSET < transform.position.x)
			FlappyObstaclesSpawner.setDestroyFlag(true);
